    ...
    <div className="mt-[1rem] flex flex-col gap-[1rem] h-[calc(100%_-_3rem)] overflow-y-auto scrollbar-blue pr-3">
      {
        adList.map((info) => <div key={info.id} className="flex gap-[1rem] bg-blue-100 items-center py-[0.5rem] px-[1rem] rounded-2xl">
          ...
          <button onClick={() => navigate(`/buy/${info.id}`)} className="ml-[auto] px-[1rem] py-[0.5rem] bg-blue-300 rounded-xl transition-all hover:bg-blue-600 hover:shadow-xl hover:shadow-blue-600/10 hover:text-white">{info.state === 'free'?'购买':'预定'}</button>
        </div>)
      }
    </div>
  </div>
